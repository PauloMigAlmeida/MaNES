...
use crate::{ZeroPage, ZeroPageX, Absolute, AbsoluteX, Accumulator};
...
pub fn asl(cpu: &mut Mos6502, inst: Instruction, bus: &mut Bus) -> u8 {
    ...
    if inst.mode == Accumulator{
        ...
        bus.write_u8(addr, (result & 0x00ff) as u8);
    }
...
mod tests {
    ...
    fn zero_page() {
        ...
        bus.write_u8(0x10, 0b0000_0001);
        let cycles = cpu.execute_instruction(opcode.opcode, &mut bus);
        assert_eq!(cycles, opcode.cycles);
        assert_eq!(cpu.x, 0);
        assert_eq!(bus.read_u8(0x10), 0b0000_0010);
        assert_eq!(cpu.flags, 0b0000_0000);
    ...
    fn zero_page_x() {
        ...
        cpu.x = 0x01;
        bus.write_u8(cpu.pc + 1, 0x10);
        bus.write_u8(0x11, 0b0000_0001);
        let cycles = cpu.execute_instruction(opcode.opcode, &mut bus);
        assert_eq!(cycles, opcode.cycles);
        assert_eq!(cpu.x, 0x1);
        assert_eq!(bus.read_u8(0x11), 0b0000_0010);
        assert_eq!(cpu.flags, 0b0000_0000);
        assert_eq!(cpu.pc, 0x0802);
        assert_eq!(cpu.sp, 0xff);
    }
    ...
    fn absolute() {
        ...
        bus.write_u8(0x1234, 0b0000_0001);
        let cycles = cpu.execute_instruction(opcode.opcode, &mut bus);
        assert_eq!(cycles, opcode.cycles);
        assert_eq!(bus.read_u8(0x1234), 0b0000_0010);
        assert_eq!(cpu.x, 0x0);
        assert_eq!(cpu.a, 0x0);
        assert_eq!(cpu.flags, 0b0000_0000);
    ...
    fn absolute_x() {
        ...
        cpu.pc = 0x0800;
        bus.write_u16(cpu.pc + 1, 0x1234);
        bus.write_u8(0x1235, 0b0000_0001);
        let cycles = cpu.execute_instruction(opcode.opcode, &mut bus);
        assert_eq!(cycles, opcode.cycles);
        assert_eq!(bus.read_u8(0x1235), 0b0000_0010);
        assert_eq!(cpu.a, 0x0);
        assert_eq!(cpu.x, 0x1);
        assert_eq!(cpu.flags, 0b0000_0000);
        assert_eq!(cpu.pc, 0x0803);
        ...
        cpu.pc = 0x0800;
        bus.write_u16(cpu.pc + 1, 0x1234);
        bus.write_u8(0x1333, 0b0000_0001);
        let cycles = cpu.execute_instruction(opcode.opcode, &mut bus);
        assert_eq!(cycles, opcode.cycles);
        assert_eq!(cpu.a, 0x0);
        assert_eq!(cpu.x, 0xff);
        assert_eq!(bus.read_u8(0x1333), 0b0000_0010);
        assert_eq!(cpu.flags, 0b0000_0000);
        assert_eq!(cpu.pc, 0x0803);
        assert_eq!(cpu.sp, 0xff);
    }
